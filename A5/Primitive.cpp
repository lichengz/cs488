...
using namespace std;
using namespace glm;
double intersection_threshold_2 = 0.01;

int cubeIndexPair[12][3] = {
    {0, 1, 2},//bot
	{2, 3, 0},
	{0, 4, 7},//left
    {7, 3, 0},
...
double checkTriInterct(const Ray ray, glm::vec3 P0, glm::vec3 P1, glm::vec3 P2) {
	...
	if((t>intersection_threshold_2) && (beta >= 0) && (gamma >= 0) && (beta + gamma <= 1) ){
...
bool rootsValidation(size_t res, double roots[2]){
    ...
    if(std::max(roots[0], roots[1]) < intersection_threshold_2)
    {
        return false;
    }
...
intersection Cube::checkIntersection(const Ray & ray){
    ...
    int_res.t = -1;
    for (int i = 0; i < 11; i++){
        double  temp_res = checkTriInterct(ray, cube_vertex_nonhierBox[cubeIndexPair[i][0]],
                                        cube_vertex_nonhierBox[cubeIndexPair[i][1]],
                                        cube_vertex_nonhierBox[cubeIndexPair[i][2]]);
        if(temp_res >= intersection_threshold_2 && (int_res.t == -1 || (temp_res < int_res.t ))){
            int_res.t = temp_res;
            switch (i){
                case (0):
                case (1):
